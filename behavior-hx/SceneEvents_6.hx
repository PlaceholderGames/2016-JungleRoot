	...
	override public function init()
	{
		...
		runPeriodically(1000 * 1, function(timeTask:TimedTask):Void
		{
			if(wrapper.enabled)
			{
				...
				{
					createRecycledActor(getActorType(46), getActor(46).getX(), getActor(46).getY(), Script.MIDDLE);
					getLastCreatedActor().applyImpulse(1, 0, 50);
				}
			}
		}, null);
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(isInRegion(getActor(1), getRegion(2)))
				{
					getActor(1).setFriction(0);
				}
			}
		});
